...
def find_path(graph):
    ...
    for edge in graph:
        key = edge[0]
        if key in graph_map:
            graph_map[key].append(edge)
        else:
            graph_map[key] = [edge]

    for path in graph:
        count += len(graph_map[path[1]])
...
def main_func(string):
    ...
    if direction == "1":
        ...
        return find_path(function_input)


if __name__ == '__main__':
    print(main_func("3 12 23 34 41 31 52 45 61 14 21 33 55 13 54 32 56 36"))
